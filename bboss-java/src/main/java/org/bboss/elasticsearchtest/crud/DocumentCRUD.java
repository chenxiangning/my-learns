...
//		documentCRUD.testBulkAddDocuments();
		...
		documentCRUD.testCreateCxn();
//		documentCRUD.testinsert();
        documentCRUD.testinsert1();
    }

    public void testinsert1() {
        //创建加载配置文件的客户端工具，单实例多线程安全
//		ClientInterface clientUtil = ElasticSearchHelper.getConfigRestClientUtil("esmapper/cxn-demo.xml");
        ClientInterface clientUtil = ElasticSearchHelper.getRestClientUtil();
        try {

            List<CxnDO> cxnDOS = new ArrayList<>();

            for (int i = 20; i < 30; i++) {
                CxnDO cxnDO = new CxnDO();
                cxnDO.setCxnBody("xxx");
                cxnDO.setSid((long) i);
                cxnDO.setApplicationName("xxx");
                cxnDO.setMyDate(new Date());

                cxnDOS.add(cxnDO);
            }


            String result = clientUtil.addDocuments("cxn_car", "cxn_car", cxnDOS, "refresh=true");
    ...
    public void testinsert() {
        ...
        try {
            ...
            CxnDO cxnDO = new CxnDO();
            cxnDO.setCxnBody("我的陈湘宁");
            cxnDO.setSid(200L);
            cxnDO.setApplicationName("setApplicationName");
            cxnDO.setMyDate(new Date());
//			String result = clientUtil.addDocument("cxn_car", cxnDO);
//			System.out.println(result);

            String response = clientUtil.addDocumentWithId("cxn_car","cxn_car",cxnDO, 200L);
    ...
    public void testCreateCxn() {
        //创建加载配置文件的客户端工具，单实例多线程安全
        ClientInterface clientUtil = ElasticSearchHelper.getConfigRestClientUtil("esmapper/cxn-demo.xml");
        try {

            boolean exist = clientUtil.existIndice("cxn_car");
            if (exist) {
                String r = clientUtil.dropIndice("cxn_car");
                System.out.println(r);
            }

            //创建索引表demo
            clientUtil.createIndiceMapping("cxn_car", "cxn_car_mp");

            String demoIndice = clientUtil.getIndice("cxn_car");//获取最新建立的索引表结构
